...
def fb_canvas(request,app_id,stat_name):
    ...
    highscore = []
    for f in friends:
        fbuser = StatsUser.objects.get_or_create(type='facebook', uid=f.get('uid'), defaults={'name': 'fb user'})
        fbuser.save()
        highscore.append(StatisticEntry.objects.filter(statistic=s).filter(user__id=fbuser.id).order_by('value')[0])
