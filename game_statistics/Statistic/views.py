...
def fb_canvas(request,app_id,stat_name):
    ...
    friends = request.facebook.fql.query("SELECT uid FROM user WHERE uid IN (SELECT uid2 FROM friend WHERE uid1 = "+request.facebook.uid+")")
    friendst.append({u'uid':request.facebook.uid})
